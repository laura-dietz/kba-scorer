...
def write_graph(path_to_write_graph, stats):
    ...
    for cutoff in sorted(stats,reverse=True):
        ...
        RecallLower.append(stats[cutoff]['R']- stats[cutoff]['RStdErr'])
        RecallUpper.append(stats[cutoff]['R']+ stats[cutoff]['RStdErr'])
    ...
    plt.fill_between(Xaxis, PrecisionLower, PrecisionUpper, color='b', alpha=0.2)
    plt.plot(Xaxis, Recall, label='Recall')
    plt.fill_between(Xaxis, RecallLower, RecallUpper,color='g', alpha=0.2)
